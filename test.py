...
st.markdown("<h1 style='color:black;'>Autism Detection</h1>", unsafe_allow_html=True)


st.markdown("<h2 style='color:black;'>Fill your details:</h2>", unsafe_allow_html=True)
st.markdown("""
    <style>
...
st.markdown("<h2 style='color:black;'>Autism Disorder Test</h2>", unsafe_allow_html=True)                                      
...
if not q1 or not q2 or not q3 or not q4 or not q5 or not q6 or not q7 or not q8 or not q9 or not q10 or not q11 or not q12:
    # print("⚠️ Please fill all inputs before proceeding.")
     st.warning("⚠️ Please fill all inputs before proceeding.")
     st.stop()
# q is scalar 
else:
...
if (st.button("Predict")):
    if 'valuef' not in locals():
        st.error("Please fill all inputs first.")
        # st.stop()
    ...
    if output == 1:
        st.success("✅ High chances of autism.")
        st.info("For any treatments or suggestions, you can ask NeuroBot in the sidebar.")
    else:
        st.success("🧠Less chances of autism.")

with st.sidebar:
    st.title("🤖 NeuroBot")

    load_dotenv()
    os.environ["GROQ_API_KEY"] = "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"
